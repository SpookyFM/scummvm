...
bool BackgroundAnimationBlob::GetIsAutoUpdated(bool bpp6, uint16_t bpp8) const {
	...
	*/
	...
	uint16_t bp24 = stream.readUint16();
	if (cx > bp24) {
		// l00B7_156A:
		cx = 1;
	}
	...
	for (; cx > 0; cx--) {
		...
		uint16_t bp1A = stream.readUint16();
		uint16_t bp1C = stream.readUint16();
		stream.seek(0x2, SEEK_CUR);
		uint16_t bp16 = stream.readUint16();
		uint16_t bp18 = stream.readUint16();
