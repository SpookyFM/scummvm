...
void Macs2Engine::readResourceFile() {
	...
	_borderHighlightSprite.Data = Common::Array<uint8_t>(_borderHighlightData, _borderHighlightWidth * _borderHighlightHeight);
...
void Macs2Engine::changeScene(uint32_t newSceneIndex, bool executeScript) {
	...
	uint16_t sceneDataOffset = newSceneIndex * 0xC;
	...
	uint32_t sceneDataOffset2 = _fileStream->readUint32LE();
...
bool Macs2Engine::GetPathfindingOverride(uint16_t index, uint16_t& result) {
...
void Macs2Engine::SetPathfindingOverride(uint16_t index, uint16_t overrideValue) {
...
uint8_t Macs2Engine::GetPathfindingOverride2(uint16_t index) {
...
void Macs2Engine::RemovePathfindingOverride(uint16_t index){
...
void Macs2Engine::DumpStream(Common::MemoryReadStream *s, uint16_t len) {
	int64 start_pos = s->pos();
	Common::String result;
	for (int i = 0; i < len; i++) {
		uint8_t v = s->readByte();
...
void Macs2Engine::loadAnimationFromSceneData(uint16_t objectIndex, uint16_t slotIndex, uint8_t arrayIndex) {
	// We need to account for the game starting indices at 1
	uint32_t address = array520D[arrayIndex - 1];
...
void Macs2Engine::loadSongFromSceneData(uint8_t dataIndex) {

	uint32_t address = array520D[dataIndex - 1];
	_fileStream->seek(address);
	uint32_t size = _fileStream->readUint32LE();
	// TODO: Also delete this one
	Common::Array<uint8_t> *data = new Common::Array<uint8_t>();
	data->resize(size);
	_fileStream->read(data->data(), size);
...
Common::Point AnimFrame::GetBottomMiddleOffset(uint16_t scale) const {
	if (scale == 100) {
...
uint16_t BackgroundAnimationBlob::Func168C(Common::Array<uint8_t> &blob) {
	// [bp-2h]
	uint16_t result;
	Common::MemoryReadStream *stream = new Common::MemoryReadStream(blob.data(), blob.size());
	stream->seek(0xA);
	uint16_t bp4 = stream->readUint16LE();
	bp4++;
	stream->seek(0xA + bp4 - 2);
	uint8_t bp6 = stream->readByte();
	if (bp4 == 0x3) {
		// l00B7_16AF:
		result = 0;
	}
