...
void ScriptExecutor::FuncA334(uint32_t value) {
	// TODO: Why is this byte never really used?
	uint8_t b = ReadByte();

	uint16_t variableID = ReadWord();
...
void ScriptExecutor::FuncC8E4() {
	...
	uint32_t characterID = Func9F4D_32() - 0x400;
...
void ScriptExecutor::FuncB6BE_actual() {
	// bp-2h
	uint32_t id = Func9F4D_32();
	id -= 0x1000;
	// bp-4h
	uint16_t bp4 = Func9F4D_16();
	if (id <= 0) {
	...
	uint16_t result168C = BackgroundAnimationBlob::Func168C(blob.Blob);
...
uint16 ScriptExecutor::Func101D(uint16 x, uint16 y) {
	...
	// l0037_107F:
	uint8_t overrideValue = g_engine->GetPathfindingOverride2(result);
...
void ScriptExecutor::SetVariableValue(uint16_t index, uint32_t value) {
	uint16_t a = static_cast<uint16_t>(value >> 16);    // High 16 bits
	uint16_t b = static_cast<uint16_t>(value & 0xFFFF); // Low 16 bits
	...
	*/
	...
	l0037_DC5C:
		...
		if (opcode1 == 0x0E) {
			...
			constexpr uint32_t durationMultiplier = 50;
			// constexpr uint32_t durationMultiplier = 1;
			...
			uint16_t areaID = Func9F4D_16();
			uint16_t active = Func9F4D_16();
			uint16_t overrideValue = Func9F4D_16();
			if (active) {
			...
			uint32_t objectID = Func9F4D_32() - 0x400;
			uint32_t animationID = Func9F4D_16();
			uint32_t offset = Func9F4D_16();
			...
			//Func9F4D(throwaway1, throwaway2);
			uint32_t objectID = Func9F4D_32() - 0x400;
			uint16_t animIndex = Func9F4D_16();
			...
			uint32_t a = Func9F4D_32();
			uint32_t b = Func9F4D_32();
			...
			uint32_t objectID = Func9F4D_32() - 0x400;
			uint16_t slotID = Func9F4D_16();
			// TODO: Unknown purpose
			Func9F4D_32();
			uint8_t arrayIndex = ReadByte();
			...
			// [bp-2h]
			uint32_t v1 = Func9F4D_32() - 0x800;
			// [bp-4h]
			uint32_t v2 = Func9F4D_32() - 0x800;
	...
	void ScriptExecutor::SetCurrentSceneScriptAt(uint32_t offset) {
		SetScript(Scenes::instance().CurrentSceneScript);
