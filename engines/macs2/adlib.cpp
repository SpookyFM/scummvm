...
void Adlib::Func2839(uint8_t bpp0A, StreamHandler* sh) {
	uint8_t bp1 = gArray8d[bpp0A];
	uint8_t bp2 = gArray96[bpp0A];
			...
			if ((bp6 & 0xF0) == 0x90) {
				...
				if (bp5 != 0) {
					...
					if (g2291 != bp8) {
						...
						// l0017_1CFF:
						gArray2235[bp8] = bp4;
						uint8_t value = gArray225F[bp3];
						bp10 = Func19BE_SH(shMem2248, value << 0x4);
						uint8_t temp = bp5;
						temp &= 0x7F;
						// dx
						uint8_t temp2 = temp >> 0x1;
						temp = 0x3F;
						temp -= temp2;
						temp = temp >> 0x1;
						bp1 = temp;
						...
						uint8_t result = Func2779(gArray96[bp8] + 0x40);
...
void Adlib::LoadData(Common::MemoryReadStream *fileStream, int64 pos, uint16_t size, void *target) {
