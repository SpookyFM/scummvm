...
View1::View1() : UIElement("View1") {
	...
	void View1::drawPathfindingPoints(Graphics::ManagedSurface &s) {
		...
		Common::Array<uint8_t> &overlay = GetCharacterByIndex(1)->PathfindingOverlay;
		for (int y = 0; y < 200; y++) {
			for (int x = 0; x < 320; x++) {
...
void View1::DrawCharacters(Graphics::ManagedSurface &s) {
	...
	for (auto current : characters) {
		...
		uint16_t scalingFactor = CalculateCharacterScaling(depth);
...
void View1::DrawVerticalBorderHighlight(const Common::Point &pos, int16 height, uint8_t unknown, Graphics::ManagedSurface &s) {
...
uint16_t View1::CalculateCharacterScaling(uint16_t characterY) const {
