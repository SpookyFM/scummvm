...
void View1::DrawSprite(int16 x, int16 y, uint16 width, uint16 height, byte* data, Graphics::ManagedSurface& s, uint8_t depth)
{
...
void View1::DrawSprite(const Common::Point &pos, uint16 width, uint16 height, byte *data, Graphics::ManagedSurface &s, uint8_t depth) {
...
void View1::DrawCharacters(Graphics::ManagedSurface &s) {
	...
	for (auto current : characters) {
		...
		uint8_t depth = g_engine->_depthMap.getPixel(current->GetPosition().x, current->GetPosition().y);
